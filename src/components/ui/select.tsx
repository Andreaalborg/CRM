...
>(({ className, children, error, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-11 w-full items-center justify-between gap-2 rounded-lg border-2 bg-[var(--color-card)] px-4 py-2 text-base text-[var(--color-foreground)] transition-all duration-200",
...
>(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        "relative z-50 max-h-96 min-w-[8rem] overflow-hidden rounded-xl border-2 border-[var(--color-border)] bg-[var(--color-popover)] text-[var(--color-foreground)] shadow-lg",
