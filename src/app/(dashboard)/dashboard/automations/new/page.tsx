...
import { 
  ...
  ArrowRight, Trash2, Settings, ChevronDown, AlertCircle,
...
const triggerTypes = [
  { 
    ...
    label: "Skjemainnsending", 
    description: "Når noen fyller ut et skjema",
    icon: Users,
    color: "#6366f1"
  },
  { 
    value: "FIELD_VALUE", 
    label: "Feltverdi", 
    description: "Når et felt har en spesifikk verdi",
    icon: Filter,
    color: "#8b5cf6"
  },
  { 
    ...
    label: "Statusendring", 
    description: "Når en lead endrer status",
    icon: CheckCircle,
    color: "#10b981"
  },
  { 
    value: "DATE_FIELD", 
    label: "Datofelt", 
    description: "Basert på dato (bursdag, jubileum, etc.)",
    icon: Calendar,
    color: "#f59e0b"
  },
  { 
    value: "INACTIVITY", 
    label: "Inaktivitet", 
    description: "Når en lead ikke har aktivitet på X dager",
    icon: AlertCircle,
    color: "#ef4444"
  },
  { 
    value: "RECURRING", 
    label: "Gjentakende", 
    description: "Kjør regelmessig (daglig, ukentlig, etc.)",
    icon: Clock,
    color: "#06b6d4"
  },
];
...
const actionTypes = [
  { 
    value: "SEND_EMAIL", 
    label: "Send e-post", 
    description: "Send en e-post til mottaker",
    icon: Mail,
    color: "#6366f1"
  },
  { 
    value: "WAIT_DELAY", 
    label: "Vent", 
    description: "Vent en periode før neste handling",
    icon: Clock,
    color: "#f59e0b"
  },
  { 
    value: "UPDATE_SUBMISSION_STATUS", 
    label: "Oppdater status", 
    description: "Endre status på lead",
    icon: CheckCircle,
    color: "#10b981"
  },
  { 
    ...
    label: "Send varsel", 
    description: "Send intern varsling",
    icon: Bell,
    color: "#8b5cf6"
  },
  { 
    value: "CONDITION", 
    label: "Betingelse", 
    description: "Hvis/ellers basert på data",
    icon: GitBranch,
    color: "#06b6d4"
  },
];
...
export default function NewAutomationPage() {
  ...
  function updateAction(id: string, config: Record<string, unknown>) {
  ...
  return (
    <>
      <Header 
        title="Ny automasjon"
        description="Bygg en automatisert workflow"
      />
            ...
            <Card style={{ marginBottom: "24px" }}>
              ...
              <CardContent>
                <p style={{ color: "#6b7280", marginBottom: "16px" }}>
                  Hva skal starte denne automasjonen?
                </p>
                      ...
                      <div>
                        ...
                        <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                          <Input
                            ...
                            style={{ width: "100px" }}
                          />
                          <span style={{ color: "#6b7280" }}>dager (negativ = før, positiv = etter)</span>
                        </div>
                      </div>
                    </div>
                  )}
            ...
            <Card style={{ marginBottom: "24px" }}>
              ...
              <CardContent>
                  ...
                  <div>
                    ...
                    {selectedFormId && (
                      <p style={{ fontSize: "12px", color: "#6b7280", margin: "2px 0 0 0" }}>
                        Skjema: {forms.find(f => f.id === selectedFormId)?.name}
                      </p>
                    )}
                  </div>
                </div>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                          <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                            ...
                            <Input
                              ...
                              onChange={(e) => updateAction(action.id, { ...action.config, field: e.target.value })}
                              placeholder="felt"
                              style={{ width: "120px" }}
                            />
                            <select
                              value={action.config.operator as string || "equals"}
                              onChange={(e) => updateAction(action.id, { ...action.config, operator: e.target.value })}
                              style={{ padding: "10px", borderRadius: "8px", border: "1px solid #e2e8f0" }}
                            >
                              <option value="equals">er lik</option>
                              <option value="contains">inneholder</option>
                              <option value="not_equals">er ikke lik</option>
                              <option value="greater_than">er større enn</option>
                              <option value="less_than">er mindre enn</option>
                            </select>
                            <Input
                              value={action.config.value as string || ""}
                              onChange={(e) => updateAction(action.id, { ...action.config, value: e.target.value })}
                              placeholder="verdi"
                              style={{ width: "120px" }}
                            />
                          </div>
                ...
                <div style={{
                  ...
                  <p style={{ textAlign: "center", fontWeight: "500", marginBottom: "16px" }}>
                    Legg til handling
                  </p>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              <div style={{ display: "flex", gap: "12px" }}>
                ...
                <Button onClick={handleSave} disabled={isSaving}>
                  {isSaving ? "Lagrer..." : "Aktiver automasjon"}
                </Button>
              </div>
            </div>
          </div>
